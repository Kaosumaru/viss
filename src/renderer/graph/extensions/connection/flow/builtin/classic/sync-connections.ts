...
function findPort<Schemes extends ClassicScheme>(
  socket: SocketData,
...
function findConnections<Schemes extends ClassicScheme>(
  socket: SocketData,
  editor: NodeEditor<Schemes>
) {
 ...
 */
export function syncConnections<Schemes extends ClassicScheme>(
  sockets: SocketData[],
  editor: NodeEditor<Schemes>
) {
  const connections: Schemes["Connection"][] = sockets
    .map((socket) => {
