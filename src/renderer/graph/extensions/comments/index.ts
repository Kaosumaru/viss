...
export type Produces =
  ...
  | {
      ...
      data: { id: Comment["id"]; dx: number; dy: number; sources?: NodeId[] };
    }
  ...
  setParent(scope: Scope<BaseArea<Schemes> | K>): void {
    ...
    this.addPipe(async (context) => {
          ...
          for (const [, comment] of linkedComments) {
            if (comment instanceof InlineComment) {
              this.area.area.content.reorder(
                comment.element,
                matchedView[1].element.nextElementSibling
              );
            }
      ...
      if (context.type === "nodetranslated") {
        ...
        await Promise.all(
          comments
            ...
            .map(async (comment) => {
              if (
                comment instanceof InlineComment &&
                !commentTracker.isTranslating(comment.id)
              ) {
                await commentTracker.translate(comment.id, dx, dy, [id]);
              }
              if (
                comment instanceof FrameComment &&
      ...
      if (context.type === "commenttranslated") {
        ...
        await Promise.all(
          comment.links
            ...
            .map((linkId) => ({
              linkId,
              view: this.area.nodeViews.get(linkId),
            }))
      ...
      if (context.type === "nodedragged") {
        ...
        comments
          .filter(
            (comment): comment is FrameComment =>
              comment instanceof FrameComment
          )
          .forEach((comment) => {
