...
export function nodesBBox<S extends ExpectedSchemes>(
  ...
  const rects = ids
    .map((id) => {
      ...
      const { width, height } = editor.getNode(id) as {
        width: number;
        height: number;
      };
  ...
  const right =
    Math.max(...rects.map((rect) => rect.position.x + rect.width)) +
    marginRect.right;
  const bottom =
    Math.max(...rects.map((rect) => rect.position.y + rect.height)) +
    marginRect.bottom;
...
export function trackedTranslate<S extends ExpectedSchemes, T>(props: {
  area: BaseAreaPlugin<S, T>;
}): {
...
export function trackedTranslateComment<
  CommentType extends TranslatableComment
>(
  ...
  const increment = (id: string) =>
    activeTranslations.set(id, (activeTranslations.get(id) ?? 0) + 1);
